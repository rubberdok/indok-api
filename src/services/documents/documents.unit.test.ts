...
describe("Documents service", () => {
  ...
  describe("#find", () => {
    it("returns the matching document if the user has permission", async () => {
      ...
      permissions.hasFeaturePermission.mockResolvedValueOnce(true)
      repository.documents.find.mockResolvedValueOnce(Result.success({ document: makeDocument() }))

      const result = await service.find(makeMockContext({ id: faker.string.uuid() }), { id: faker.string.uuid() })

      expect(result).toEqual(Result.success({ document: expect.any(Document) }))
    })
    ...
    it("returns UnauthorizedError if the user is not logged in", async () => {
      ...
      const result = await service.find(makeMockContext(null), { id: faker.string.uuid() })
