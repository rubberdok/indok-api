...
function buildDocuments({
  ...
  return {
    ...
    async find(ctx, params) {
      ...
      try {
        const document = await db.document.findUnique({ where: { id: params.id }, include: { categories: true } })
        if (document === null) return Result.error(new NotFoundError("Document does not exist"));
        return Result.success({ document: new Document(document) });
      } catch (err) {
        return Result.error(
