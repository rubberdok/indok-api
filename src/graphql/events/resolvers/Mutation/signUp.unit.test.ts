...
import { Event, EventSignUp, ParticipationStatus } from "@prisma/client";
  ...
  describe("signUp", () => {
    it("should sign the user up for an event", async () => {
      ...
      eventService.signUp.mockResolvedValue(
        mock<EventSignUp>({ id: faker.string.uuid(), participationStatus: ParticipationStatus.CONFIRMED })
      );
      ...
      expect(eventService.signUp).toHaveBeenCalledWith(contextValue.req.session.userId, expect.any(String));
    });
